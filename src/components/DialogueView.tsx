        ...
        <>
          ...
          {selected && (
            <>
                  ...
                  {QUESTIONS.map((q, i) => (
                    ...
                    >
                      <div className="flex items-start gap-3">
                        <span className={`flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center text-xs font-semibold ${activeQ === i ? 'bg-brand-600 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400'}`}>
                          {i + 1}
                        </span>
              ...
              {/* Comment thread */}
              <div className="card">
                <h3 className="font-semibold text-gray-800 dark:text-gray-100 text-sm mb-3">{t('dialogue.notes_label')}</h3>
                ...
                {(selected.comments?.length ?? 0) === 0 ? (
                  ...
                  <div className="space-y-3 mb-4">
                    {selected.comments!.map(c => (
                      <div key={c.id} className="bg-gray-50 dark:bg-gray-700 rounded-lg px-3 py-2.5">
